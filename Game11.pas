...
const
 tipo : TPoint = (X:138;Y:452);


var
 ju11 : TGame11;

constructor TGame11.Create;
 var
  i : byte;
 begin
  inherited Create(Screen,KeyBoard,Sound,Mouse);
  lib := TILib.Create(scr,'Lib\Juego9.lib');
  SndLib := TSoundLibrary.Create('Lib\Juego11.snd',Sound);
...
procedure TGame11.PrepareRenderTriste;
 begin
  Prepare(False);
...
procedure TGame11.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TGame11.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
 begin
  Right := false;
  Left := false;
  Up := false;
...
procedure TGame11.UpdateGame(Value : boolean);
 var
  i : integer;
 begin
...
procedure TGame11.UpdateObject(Objected : PObjectUpdate);
 begin
  with Objected^ do
   scr.surface.draw(cX,cY,images[cIndex].surface,Transparent);
 end;

procedure TGame11.CreateObject(xx,yy,iindex : integer; trans : boolean);
 var
...
procedure TGame11.AnimaPiedras;
 var
  pegado : boolean;
 begin
  scr.surface.draw(cxp,194,images[7].surface,true);
  if (GetTickCount - tick < speed) then exit;
  tick := GetTickCount;
  if (cxp=158) or (cxp=416) then sig := -sig;
  inc(cxp,sig);
  if salto or (options = RenderCaida) then exit;
  ...
  if ((cxr >= tipo.X) and (cxr <= tipo.Y)) and (cyr=146) then
   begin
    tc := 2; salto := false;
    options := RenderCaida;
    KeyTick := GetTickCount;
...
function TGame11.RenderGame : byte;
 begin
  result := 0;
  key.Update;
  case options of
...
procedure TGame11.updateKeyboard;
 begin
...
function TGame11.RenderLoad : boolean;
 begin
...
function CoCreateGame(Screen : TDXDraw; KeyBoard : TDXInput; Sound : TSoundSystem; Mouse : Mouse.TMouse) : TFarProcObj; stdcall;
 begin
  result := nil;
  ju11 := TGame11.Create(Screen,KeyBoard,Sound,Mouse);
  result := ju11.RenderGame;
 end;
